...
namespace Watermelon.BusStop
{
    ...
    public class DockBehavior : MonoBehaviour
    {
        ...
        public GameObject slot1,slot1Locked,slot2,slot2Locked;
        public GameObject VipSlotUnlocked, VipSlotLocked;
        public void UnlockVipSlot()
        {
            PlayerPrefs.SetInt("VipSlotUnlocked", 1);
            VipSlotUnlocked.SetActive(true);
            VipSlotLocked.SetActive(false);
            VipSlotUnlocked.GetComponent<SlotBehavior>().Clear();
            ResetSlots();
        }
        public void Init(LevelController levelController)
        {
           ...
           /* if(LevelController.DisplayLevelNumber + 1 >= 3)
            {
        ...
        public bool CheckMatch(bool remove = true)
        {
        ...
        public bool CheckDockMatch(bool remove = true)
        {
            int counter = 1;
        ...
        private bool waitBeforeCheckingFilled;

        private IEnumerator DelayedCheckSlotsFilled()
        {
            waitBeforeCheckingFilled = true;
            yield return new WaitForSeconds(0.1f); // Adjust delay as needed
            waitBeforeCheckingFilled = false;
        ...
        {
            if (Time.frameCount % 15 != 0 || IsEmpty || waitBeforeCheckingFilled)
                return;

            if (!GameController.Data.ActivateVehicles)
            {
                StartCoroutine(DelayedCheckSlotsFilled());
            }
            else
            {
                CheckBusMatch();

                if (IsFilled && !EnvironmentBehavior.IsCollectingPlaceAvailable && !ShouldWaitForBus())
                {
                    StartCoroutine(DelayedCheckSlotsFilled());
                }
            }
        ...
        public void OnMoveEnded(SlotCase slotCase, MoveType moveType)
        {
            ...
            if (addedDepth > 0 && addedSlotCase != slotCase && !slots[^3].IsOccupied)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    slot.Clear();
                   // Destroy(slot.gameObject);// Destroying slot
                }

                addedDepth--;
            }
        }
        public int GetUnoccupiedSlotCount()
        {
            int count = 0;
            for (int i = 0; i < slots.Count; i++)
            {
        ...
        public bool HasUnoccupiedSlots()
        {
            int unoccupiedCount = GetUnoccupiedSlotCount();
            Debug.Log($"Unoccupied Slots: {unoccupiedCount}");
            return unoccupiedCount > 0;
        }
        private void Update()
        {
         /*   int unoccupiedCount = GetUnoccupiedSlotCount();
